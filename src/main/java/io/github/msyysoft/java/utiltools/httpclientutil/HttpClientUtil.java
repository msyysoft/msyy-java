    ...
    static {
        try {
            if (HCBProxyConfig.isProxy_switch()) {
                client4HTTP = HCB.custom().proxy(HCBProxyConfig.getProxy_ip(), HCBProxyConfig.getProxy_port()).build();
                client4HTTPS = HCB.custom().ssl().proxy(HCBProxyConfig.getProxy_ip(), HCBProxyConfig.getProxy_port()).build();
            } else {
